<template>
  <div class="clearfix">
    ...
    <Row>
      <Col span="4" offset="8">
      <Button type="primary" @click="onLineReview">上线审批</Button>
      </Col>
      <Col span="4">
      <Button type="primary" @click="onDelete">项目作废</Button>
      </Col>
    </Row>
    <Row :gutter="20">
        ...
        <Tabs class="home-push">
          <TabPane label="基本信息" name="0" style="padding:10px;">
            <ul class='wrap'>
              ...
              <li>项目标题：
                <span>{{ baseAndProList.proname }}</span>
              </li>
              <li>项目概况：
                <span>{{ baseAndProList.prodeclare}}</span>
              </li>
            </ul>
          </TabPane>
        </Tabs>
      ...
      <Card dis-hover>
        <Tabs class="home-push">
          <TabPane label="项目信息" name="0" style="padding:10px;">
            <ul class='wrap'>
              <li>项目编号：
                <span>{{ baseAndProList.proid }}</span>
              </li>
              <li>创建时间：
                <span>{{ baseAndProList.createdate }}</span>
              </li>
              <li>发起人：
                <span>{{ baseAndProList.creater }}</span>
              </li>
              <li>项目状态：
                <span>{{ baseAndProList.prostate }}</span>
              </li>
              <li>预计上线时间：
                <span>{{ baseAndProList.plansdate }}</span>
              </li>
              <li>预计下线时间：
                <span>{{ baseAndProList.planedate }}</span>
              </li>
              <li>预计工期：
                <span>{{ baseAndProList.betweenDays }}</span>
              </li>
            </ul>
          </TabPane>
        </Tabs>
          ...
          <TabPane label="开发日志" name="1" style="padding:10px;">
            <div style="margin-top:10px;">
              <Row style="margin-bottom:10px;">
                ...
                <Col span="8">
                <Select placeholder="二级类型：" @on-change="onChangeLevel" style="width:200px" v-show="showSecondLevel">
                  <Option v-for="item in secondLevelData" :key="item.taskId" v-model:value="item.taskId">{{item.taskName}}</Option>
                </Select>
                </Col>
                <Col span="8">
                <Select placeholder="二级类型：" @on-change="onChangeSubLevel" style="width:200px" v-show="subshowSecondLevel">
                  <Option v-for="item in subsecondLevelData" :key="item.subtaskId" v-model:value="item.subtaskId">{{ item.subtaskId }}</Option>
                </Select>
                </Col>
              </Row>
              <Row>
      ...
      <Card dis-hover>
        <Tabs class="home-push">
          <TabPane label="参与组" name="0" style="padding:10px;">
            ...
            <Table border height="500" :data="devDetails.taskList" :columns="squadColumns" class="apply-squad">
            </Table>
          </TabPane>
        </Tabs>
      </Card>
    ...
    <Modal v-model="addshow" :title="addmodaltitle" @on-cancel="cancel('partInParams')">
...
import { passOrReject } from '../../../api/myproject';
...
export default {
  ...
  methods: {
    ...
    onTypeChange(value) {
      ...
      this.subsecondLevelData = [];
      switch (currvalue) {
        case '1':
          this.showSecondLevel = false;
          this.subshowSecondLevel = false;
          break;
        case '2':
      ...
      getSecondLeverType(params).then(res => {
        if (res.data != null) {
          if (params.type == 2) {
            ...
            this.subsecondLevelData = res.data.data;
          }
        } else {
          this.secondLevelData = [];
          this.$Message.error('无二级类型数据');
        }
      })
    },
    addDevLogClick() {
